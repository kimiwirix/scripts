...
q$community <- factor(q$community, levels = communities)
...
#+ temperaturas con su réplica. Para el plot se está tomadno la mean de las tres 
#+ curvas 

p <- ggplot()+
  geom_line(data = q, aes(x = hrs, y = log10(m_Fragmentos_16S_ml)  , colour = as.factor(temp), group = interaction(community, repbio, temp), linetype=as.factor(repbio)), linewidth=0.8)+
